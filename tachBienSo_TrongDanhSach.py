...
for img_item in file_list:
    ...
    if screenCnt is not None:
        ...
        # cv2.imshow("alo", anh_kytu)
        # if not os.path.exists('datasetMotorcycle/'):
        #     os.mkdir('datasetMotorcycle/')
        # anh_kytu = cv2.resize(anh_kytu, dsize=(760, 560))
        # cv2.imwrite('datasetMotorcycle/' + "motorcycle_img" + str(dem) + ".jpg", anh_kytu)
        # dem = dem + 1

        # cv2.imshow("alo", anh_kytu)
        # if not os.path.exists('dataRac_1/'):
        #     os.mkdir('dataRac_1/')
        # anh_kytu = cv2.resize(anh_kytu_bw, dsize=(760, 560))
        # cv2.imwrite('dataRac_1/' + "motorcycle_img" + str(dem) + ".jpg", anh_kytu)
        # dem = dem + 1
        # img_draw = cv2.drawContours(img, [screenCnt], -1, (0, 255, 0), 2)
        # cv2.imshow("bienso", img)

        if not os.path.exists('data1/'):
            os.mkdir('data1/')
        anh_kytu = cv2.resize(anh_kytu_bw, dsize=(760, 560))
        cv2.imwrite('data1/' + "img_" + str(dem) + ".jpg", anh_kytu)
        dem = dem + 1
